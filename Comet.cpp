...
void Usage(char *pszCmd)
{
   ...
   logout("\n");
   sprintf(szErrorMsg, " SLM-Index Beta (Comet) version \"%s\"\n %s\n", comet_version, copyright);
   logout(szErrorMsg);
   logout("\n");
   sprintf(szErrorMsg, " SLM-Index Beta (Comet) usage:  %s [options] <input_files>\n", pszCmd);
   logout(szErrorMsg);
   logout("\n");
      ...
      ICometSearchManager *pSearchMgr)
{
   ...
   {
      sprintf(szErrorMsg, "\n SLM-Index Beta (Comet) version %s\n\n", comet_version);
      ...
      if (fgets(szParamBuf, SIZE_BUF, fp) != NULL)
      {
         ...
         if ( (pStr = strchr(szParamBuf, '=')) != NULL)
         {
            ...
            *pStr = 0;                     // Null rest of szParamName at equal char.
            ...
            else if (!strcmp(szParamName, "pcnt_matches"))
            {
               sscanf(szParamVal, "%lf", &dDoubleParam);
               szParamStringVal[0] = '\0';
               sprintf(szParamStringVal, "%lf", dDoubleParam);
               pSearchMgr->SetParam("pcnt_matches", szParamStringVal, dDoubleParam);
            }
            ...
            else if (!strcmp(szParamName, "max_var_mods"))
            {
               sscanf(szParamVal, "%d", &iIntParam);
               szParamStringVal[0] = '\0';
               sprintf(szParamStringVal, "%d", iIntParam);
               pSearchMgr->SetParam("max_var_mods", szParamStringVal, iIntParam);
            }
            ...
            else if (!strcmp(szParamName, "decoy_search"))
            {
               ...
               szParamStringVal[0] = '\0';
               sprintf(szParamStringVal, "%d", iIntParam);
               pSearchMgr->SetParam("decoy_search", szParamStringVal, 0);
            }
...
# SLM-Index Beta (COMET) MS/MS search engine parameters file.\n\
...
\n\
pcnt_matches = 10.0                    # Percent of Peaks to match for candidacy\n\
num_threads = 0                        # 0=poll CPU to set num threads; else specify num threads directly (max %d) [Only for Indexing)\n\
\n", MAX_THREADS);
...
\n\
#\n\
# The capacity of SLM-Index Beta\n\
#\n\
max_variable_mods = 40000000           # Default 40 Million \n\
\n\
#\n\
# fragment ions\n\
#\n\
...
# misc parameters\n\
#\n\
digest_mass_range = 100.0 10000.0      # MH+ peptide mass range to analyze\n\
