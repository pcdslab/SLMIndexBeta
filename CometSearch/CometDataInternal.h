...
#define MIN_PEPTIDE_LEN             3       /// HM: Atleast 65 peaks required for BWA based search.
...
typedef struct sDBEntry
{
   string strName;
   string strSeq;
   int iSeqFilePosition;           /* HM: TODO Should be removed as we are not doing decoys yet */
   unsigned char  iLenPep;         /* MAX_PEP_LEN = 64                      */

   sDBEntry& operator=(const sDBEntry &rhs)
   {
      this->strName = rhs.strName;
      this->strSeq = rhs.strSeq;
      this->iSeqFilePosition = rhs.iSeqFilePosition;
      this->iLenPep = rhs.iLenPep;
      return *this;
   }
} sDBEntry;
...
/************ HM: My data structures ****************/

/* TODO: indexProtein should rather be replaced by number of peptides in the protein database entries */
/* peptideEntry to be stored in Array to recover peptides */

typedef float pepEntry;

/*
typedef struct _pepEntry
{
   float         iPepMass;          iEndPos == iProteinSeqLengthMinus1

    Operators for Sorting
   _pepEntry& operator=(_pepEntry rhs)
   {
     this->iPepMass = rhs.iPepMass;
     return *this;
   }

   inline bool operator==( const _pepEntry& rhs){return this->iPepMass == rhs.iPepMass;}
   inline bool operator!=( const _pepEntry& rhs){return !(*this == rhs);}
   inline bool operator< ( const _pepEntry& rhs){return this->iPepMass < rhs.iPepMass;}
   inline bool operator> (const _pepEntry& rhs){return  this->iPepMass > rhs.iPepMass;}
   inline bool operator<=(const _pepEntry& rhs){return (*this > rhs);}
   inline bool operator>=(const _pepEntry& rhs){return (*this < rhs);}
...
{
   unsigned long long lower64 = 0x0;
   unsigned int      upper2  = 0x0;
} __attribute__ ((packed)) modSites;

/* variable Mod peptide entries to be stored in Array to recover peptides */
typedef struct _varModEntry
{
   unsigned int   indexProtein;     /* Index of the  protein DB             */
/* unsigned char  iLenPep;             MAX_PEP_LEN = 64                     */
   float          iPepMass;         /* iEndPos == iProteinSeqLengthMinus1   */
   modSites sites;

} __attribute__((packed)) varModEntry;

typedef struct _Ranks
{
   UINT occs[FIRSTROW_CHARS] = {0};
} Ranks;

typedef struct _Ranks2
{
   UINT occs[MAX_RANGE] = {0};
} Ranks2;

/******** HM: My data structures ******************************/
/* Structure to hold the BWT processed data */
typedef struct _DataBWT
{
   IDX      *BWTposition = NULL;  /* Suffix Array                   */
   Ranks     BWTfirstRow;         /* Occurences in First Row of SA  */
   Ranks2    BWTSecondRow;        /* Occurences in Second Row of SA */
} DataBWT;

typedef struct _DataRAW
{
   Peak         *dataArr = NULL;    /* Raw data LSB                            */
   Peak         *upByte  = NULL;    /* Raw data 2nd Byte                       */
   bool         isFull   = true;    /* Flag to see if more arrays needed       */
   unsigned int num_entries = 0;    /* Number of peps added into the dataArr   */
} DataRaw;

/* Structure for each Peptide in the rawdata */
typedef struct _iMSQPeptide
{
   USHORT peakArr[FRAGS];
/*   double dIntensity[FRAGS] = {0};
   double dSumIntensity = 0;*/
} iMSQPeptide;

/****************************************************************/

/********************* HM: My extern variables ******************/

/* Extern Variables - This is wrong. Should be another way */
/* vector that holds pepEntries */
extern pepEntry     *pepEntries;

/* vectors containing bwt transformed data of raw_data */
extern DataBWT bwa_data;

extern UINT         *cnt_data;

/* vector that holds var modified peptide entries */
extern varModEntry  **varModEntries;
extern vector <UINT *>     cnt_mods;

/* vectors containing bwt transformed data of raw_mods */
extern DataBWT      *bwa_mods;

/* TODO: There must be a mutex for multiprocessing on the above variables */
extern sDBEntry     *proteins;

extern int         raw_mods_size;

extern short **results;
extern short **results_mods;
...
struct Query
{
   long long  iXcorrHistogram[HISTO_SIZE];
   unsigned long long  iHistogramCount;   // # of entries in histogram
   float fPar[4];           // parameters of LMA regression

   long iMatchPeptideCount;        // # of peptide matches
   long iDecoyMatchPeptideCount;   // # of decoy peptide matches
   ...
   Query()
   {
      ...
      for(int i1 = 0; i1<FRAGS; i1++)
         _iPeaks.peakArr[i1] = 0;
...
/* HM: Some more */
extern DataRaw raw_data;
extern DataRaw *raw_mods;
extern UINT occ[256];
/************************************************/
