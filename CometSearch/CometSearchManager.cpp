...
/* HM: Create a Mutex Lock for DatabasePreprocessor
 Mutex                         g_preproecssDatabaseMutex; */
...
static bool UpdateInputFile(InputFileInfo *pFileInfo)
{
   ...
   if (bUpdateBaseName) // set individual basename from input file
   {
      ...
      if (!STRCMP_IGNORE_CASE(g_staticParams.inputFile.szFileName + iLen - 9,
            ".mzXML.gz") || !STRCMP_IGNORE_CASE(g_staticParams.inputFile.szFileName + iLen - 8, ".mzML.gz"))
      {
         if ((pStr = strrchr(g_staticParams.inputFile.szBaseName, '.')))
            *pStr = '\0';
      }
...
CometSearchManager::CometSearchManager()
{
   ...
   // Initialize the mutex we'll use to protect the database preprocess memory pool
   // Threading::CreateMutex(&g_preproecssDatabaseMutex);
...
bool CometSearchManager::InitializeStaticParams()
{
   ...
   if (GetParamValue("pcnt_matches", dDoubleData))
   {
      if (dDoubleData > 0)
      {
         g_staticParams.dPercentMatches = dDoubleData;
      }
      else
      {
         g_staticParams.dPercentMatches = 10.0;
      }
   }
   ...
   if (GetParamValue("max_var_mods", iIntData))
   {
      if (iIntData > 0)
      {
         g_staticParams.iNumberOfMods = iIntData;
      }
   }
...
bool CometSearchManager::DoSearch()
{
   ...
   if (!g_staticParams.options.bOutputSqtStream)
   {
      sprintf(szOut, " SLMIndex Beta version \"%s\"\n %s\n\n", comet_version, copyright);
      logout(szOut);
      fflush(stdout);
   }
   ...
   for (int i=0; i<(int)g_pvInputFiles.size(); i++)
   {
      ...
      if (!g_staticParams.options.bOutputSqtStream)
      {
         sprintf(szOut, " SLMIndex Beta start:  %s\n", g_staticParams.szDate);
      ...
      /* HM: Main thing starts here */
      if (bSucceeded && !donePreprocessing)
      {
         char szTimeBuffer[32];
         szTimeBuffer[0] = '\0';
         time_t PreprocessDatabaseTime;
         ...
         if (!g_staticParams.options.bOutputSqtStream)
         {
            ...
            char szOut[128];
            time(&PreprocessDatabaseTime);
            strftime(szTimeBuffer, 26, "%m/%d/%Y, %I:%M:%S %p", localtime(&PreprocessDatabaseTime));
            sprintf(szOut, " - Database Preprocess Start :  %s\n", szTimeBuffer);
            logout(szOut);
         }

         /* HM: TODO: Jugarr - Set the mass range for the peptide generation */
         ...
         bSucceeded = CometSearchManager::PreprocessDatabase();
         ...
         {
            cout << "\nPreprocessing Failed - Can't proceed\n";
            exit(0);
         }
         if (!g_staticParams.options.bOutputSqtStream)
         {
            ...
            time(&PreprocessDatabaseTime);
            strftime(szTimeBuffer, 26, "%m/%d/%Y, %I:%M:%S %p", localtime(&PreprocessDatabaseTime));
            sprintf(szOut, " - Database Preprocess End :  %s\n", szTimeBuffer);
            logout(szOut);
         }
#if (PREPROCESS == 0)
         /* Pause here */
      ...
      if (bSucceeded)
      {
         ...
         bSucceeded = CometSearch::AllocateMemory(1/*g_staticParams.options.iNumThreads*/);
         if (!bSucceeded)
         {
            break;
         }
         ...
         while (!CometPreprocess::DoneProcessingAllSpectra()) // Loop through iMaxSpectraPerSearch
         {
            ...
            /* HM: SAPPHIRE Searching Algorithm */
            ...
            {
               char szOut[128];
               int iElapsedTime = (int) difftime(end, start);
               sprintf(szOut, "     - Actual Search Time:  %d sec\n", iElapsedTime);
               logout(szOut);
            }
#endif
            ...
            bSucceeded = CometPostAnalysis::PostAnalysis(1,1/*g_staticParams.options.iNumThreads, g_staticParams.options.iNumThreads*/);
            if (!bSucceeded)
            {
               goto cleanup_results;
            }
   ...
   cleanup_results:
      ...
      CometSearch::DeallocateMemory(1/*g_staticParams.options.iNumThreads*/);
