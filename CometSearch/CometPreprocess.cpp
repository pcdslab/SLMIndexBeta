                                 ...
                                 double *pdTmpPeakExtracted)
{
   ...
   size_t iTmp= (size_t)((g_staticParams.options.dPeptideMassHigh + 100/*dCushion + 2.0*/) * g_staticParams.dInverseBinWidth)*sizeof(double);
                     ...
                     || (iPrecursorCharge == 1 && g_staticParams.options.bOverrideCharge == 3)))))
      {
               ...
               && iPrecursorCharge <= g_staticParams.options.iMaxPrecursorCharge)
         {
            ...
            if (iPrecursorCharge == 1)
               pScoring->_spectrumInfoInternal.iMaxFragCharge = 1; //g_staticParams.options.iMaxFragmentCharge;
            else
            {
               pScoring->_spectrumInfoInternal.iMaxFragCharge = iPrecursorCharge - 1;// HM: g_staticParams.options.iMaxFragmentCharge;
            ...
            pScoring->_spectrumInfoInternal.iArraySize = (int)((g_staticParams.options.dPeptideMassHigh + 100.0)/*(dMass + dCushion + 2.0)*/ * g_staticParams.dInverseBinWidth); //(g_staticParams.options.dPeptideMassHigh + 100.0)
                                   ...
                                   struct PreprocessStruct *pPre)
{
   ...
   /* HM: Will be used to populate iPeaks */
         ...
         for (ii=0; ii<iWindowSize; ii++)    // Normalize to max inten. in window.
         {
            ...
            if (iBin < pScoring->_spectrumInfoInternal.iArraySize)
            {
               if (pdTmpRawData[iBin] > dTmp2)
               {
                  ...
                  /* HM: Populate the _iPeaks here */
                  if (idx < FRAGS)
                  {
                     pScoring->_iPeaks.peakArr[idx] = (SCALE(iBin) & 0xFFFF);
/*                     pScoring->_iPeaks.dIntensity[idx] = pdTmpCorrelationData[iBin];
                     pScoring->_iPeaks.dSumIntensity += pdTmpCorrelationData[iBin];*/
                     idx++;
                  }
               }
            }
...
bool CometPreprocess::AllocateMemory(int maxNumThreads)
{
   ...
   //HM: Must be equal to largest possible array
   int iArraySize = (int)((g_staticParams.options.dPeptideMassHigh + 100.0) * g_staticParams.dInverseBinWidth); //int iArraySize = (int)((g_staticParams.options.dPeptideMassHigh + dCushion + 2.0) * g_staticParams.dInverseBinWidth);
...
bool CometPreprocess::DeallocateMemory(int maxNumThreads)
{
   ...
   ppdTmpRawDataArr = NULL;
   ppdTmpFastXcorrDataArr = NULL;
   ppdTmpCorrelationDataArr = NULL;
   ppdTmpSmoothedSpectrumArr = NULL;
   ppdTmpPeakExtractedArr = NULL;
