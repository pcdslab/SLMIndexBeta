...
void CometPostAnalysis::PostAnalysisThreadProc(PostAnalysisThreadData *pThreadData)
{
   ...
   /* HM: Since Comet didn't fix it so setting passing it to UNUSUED PARANS to avoid wwarnings */
   iMSQ_UNUSUED_PARAM(bSucceeded);
...
void CometPostAnalysis::LinearRegression(long long *piHistogram,
                                         double *slope,
                                         double *intercept,
                                         int *iMaxXcorr,
                                         int *iStartXcorr,
...
bool CometPostAnalysis::GenerateXcorrDecoys(int iWhichQuery)
{
   ...
   int iLoopMax = (DECOY_SIZE > pQuery->iHistogramCount)?(int)(DECOY_SIZE - pQuery->iHistogramCount): 0;
   bool bDecoy;
   int iLastEntry;
   ...
   /* HM: Comet is not using bDecoy anywhere so passing
    *     to UNUSED to avoid Compiler warnings */
   iMSQ_UNUSUED_PARAM(bDecoy);
         ...
         for (ii=0; ii<g_staticParams.ionInformation.iNumIonSeriesUsed; ii++)
         {
            ...
            for (ctCharge=1; ctCharge<=iMaxFragCharge; ctCharge++)
            {
               ...
               if (dFragmentIonMass < pQuery->_pepMassInfo.dExpPepMass)
               {
                  ...
                  {
                     char szErrorMsg[256];
                     sprintf(szErrorMsg,  " Error - XCORR DECOY: dFragMass %10.6f, iFragMass %d, ArraySize %d, InputMass %f, scan %d, z %d",
                           dFragmentIonMass,
                           iFragmentIonMass,
